    ...
    const regions = await page.evaluate(() => {
      const elements = Array.from(
        document.querySelectorAll(
          "#header_section > #nav > ul > li:nth-child(3) > div > ul > li > a"
        )
      );
...
const getTheatersByRegions = async (regionIndex = GANGWON_INDEX) => {
  ...
  try {
    ...
    const theatersInfo = await page.evaluate(() => {
      const elements = Array.from(
        document.querySelectorAll(
          "#nav > ul > li:nth-child(3) > div > ul > .ov > div > ul > li > a"
        )
      );
      return elements.map((element) => {
        return {
...
const getTimeTableByNaver = async (theaterName) => {
  ...
  try {
    await page.goto(
      `${NAVER_LINK}query=${encodeURI(`롯데시네마+${theaterName}`)}`,
      {
        waitUntil: "load",
        // Remove the timeout
        timeout: 0,
    ...
    const movieItems = await page.evaluate(() => {
      const items = Array.from(
        document.querySelectorAll(
          ".list_tbl_box > .list_tbl > ._wrap_time_table > tr"
        )
      );
      return items.map((item) => {
        ...
        const timeTables = Array.from(
          item.querySelectorAll("td > div > .time_info > a")
        );
