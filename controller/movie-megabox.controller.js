    ...
    const regions = await page.evaluate(() => {
      const elements = Array.from(
        document.querySelectorAll(
          ".body-wrap > .container > #contents > .inner-wrap > .theater-box > .theater-place > ul > li > .sel-city"
        )
      );
...
const getTheatersByRegions = async (regionIndex = GANGWON_INDEX) => {
  ...
  try {
    ...
    const theatersInfo = await page.evaluate(() => {
      const elements = Array.from(
        document.querySelectorAll(
          ".body-wrap > .container > #contents > .inner-wrap > .theater-box > .theater-place > ul > .on > .theater-list > ul > li > a"
        )
      );
      return elements.map((element) => {
...
const getImage = async (imageNumber) => {
  const response = await fetch(
    `https://www.megabox.co.kr/on/oh/oha/Movie/selectOneLnList.do?currentPage=1&movieNo=${imageNumber}`
  );
  ...
  try {
    ...
    const movieItems = await page.evaluate(() => {
      const items = Array.from(
        document.querySelectorAll(".reserve > .theater-list")
      );
      return items.map((item) => {
        const imageNumber = item
          .querySelector(".theater-tit > p:nth-child(2) > a")
          .getAttribute("href")
          .split("=")[1];
        const title = item.querySelector(
          ".theater-tit > p:nth-child(2)"
        ).innerText;
        const timeTables = Array.from(
          item.querySelectorAll(".theater-type-box")
        );
        const timeInfo = timeTables.map((timeTable) => {
          ...
          const timesAndSeats = Array.from(
            timeTable.querySelectorAll(
              ".theater-time > .theater-time-box > .time-list-table > tbody > tr > td"
            )
          );
          return timesAndSeats.map((timeAndSeat) => {
            const timeEle = timeAndSeat.querySelector(
              ".td-ab > .txt-center > a > .time"
            );
            const seatsEle = timeAndSeat.querySelector(
              ".td-ab > .txt-center > a > .chair"
            );
            return {
